...
class TestHairParity(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        subprocess.run(
            ["node", str(SCRIPT), str(BUILD), str(cls.dir)], check=True, cwd=REPO
        )
    ...
    def test_decoded_guides(self):
        for name, info in self.result["styles"].items():
            ...
            pts = np.fromfile(self.dir / f"{name}_points.bin", np.float32).reshape(
                style.points.shape
            )
            self.assertLess(np.abs(pts - style.points).max(), 1e-6, name)
            mir = np.fromfile(self.dir / f"{name}_mirror.bin", np.float32).reshape(
                style.points.shape
            )
            self.assertLess(
                np.abs(mir - style.mirrored(self.layout).points).max(), 1e-6, name
            )

    def test_density_volume(self):
        for name, info in self.result["styles"].items():
            style = H.load_style(name, self.layout)
            vol, lo, h = H.density_volume(
                style, self.layout, info["params"], info["count"]
            )
            nx, ny, nz = info["dims"]
            self.assertEqual(list(vol.shape[:3]), [nx, ny, nz], name)
            self.assertLess(np.abs(np.array(info["lo"]) - lo).max(), 1e-9)
            page = np.fromfile(self.dir / f"{name}_volume.bin", np.uint8).reshape(
                nz, ny, nx, 4
            )
            page = page.transpose(2, 1, 0, 3).astype(int)
            diff = np.abs(page - vol.astype(int))
            # the nearest voxel of a step can differ at a tie; the values match within one
            # step of 8 bits almost everywhere
