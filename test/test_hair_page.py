...
@unittest.skipUnless(playwright_available(), "Playwright and the built viewer page are needed")
class TestHairPage(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with sync_playwright() as p:
            ...
            cls.errors = []
            tab.on("console", lambda m: cls.errors.append(m.text) if "Shader Error" in m.text else None)
            ...
            tab.evaluate("() => window.setHairParams({length: 0.8, curl: 1.3, volume: 1.2})")
    ...
    def check_pass_b(self, rest):
        ...
        prm = rest["params"]
        ref, ell = H.strands(self.style, self.layout, guides, normals, roots, prm, count=N, scale=rest["scale"])
